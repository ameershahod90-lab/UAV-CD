...
class MissionProfileSection(ReportSection):
    ...
    def build(self, ctx: ReportContext, rb: ReportBuilder) -> None:
        ...
        rb.add_paragraph(
            "The sizing methodology follows Sadraey (2020) §2.6-2.7, where each "
            "segment contributes a weight fraction (Wi/Wi-1) to the overall "
            "MTOW buildup. Fixed segments use tabulated fractions (Table 2.4); "
            "cruise and loiter segments use the Breguet range/endurance equations."
        )
